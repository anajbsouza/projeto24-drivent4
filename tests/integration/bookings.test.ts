...
describe("GET /booking", () => {
    ...
    describe("when token is valid", () => {
        ...
        it("should respond with 400 if ticket is remote, does not include hotel or hotel is not paid", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
        ...
        it("should respond with 404 if user doesn't have a booking", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const userEnrollment = await createEnrollmentWithAddress(user);
        ...
        it("should respond with 200 and booking data if it exists", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const userEnrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketType(false, true);
...
describe("POST /booking", () => {
    ...
    describe("when token is valid", () => {
        ...
        it("should respond with 403 if roomId has a value lower than the minimum limit", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const userEnrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketType(false, true);
...
describe("PUT /booking/:bookingId", () => {
    ...
    describe("when token is valid", () => {
        it("should respond with 403 if ticket is remote, does not include hotel or hotel is not paid", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const userEnrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketType(true, false);
        ...
        it("should respond with 403 if user doesn't have a booking", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const userEnrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketType(false, true);
        ...
        it("should respond with 403 if chosen room is not available", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const userEnrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketType(false, true);
